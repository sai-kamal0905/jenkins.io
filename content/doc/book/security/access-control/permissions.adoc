...
Agent/ExtendedRead::
Allows read-only access to agent configuration and agent logs.
Users can view agent settings but cannot create, modify, or delete agents.
This permission is useful for troubleshooting and inspection without granting administrative access.

For example, a user with `Agent/ExtendedRead` permission can open
**Manage Jenkins → Nodes**, select an agent such as `linux-agent-01`,
and view its labels, executors, launch method, status, and logs.
The user cannot modify the agent configuration, change labels,
disconnect or delete the agent, or create new agents.
...
Job/ExtendedRead::
Allows read-only access to job configuration pages.
Users can view job configuration but cannot modify or save changes.
This permission is commonly used in read-only Jenkins environments.

For example, a user with `Job/ExtendedRead` permission can open the
configuration page of a job such as `build-backend-service` and view
its SCM settings, build steps, and triggers, but cannot save or modify
...
This permission is typically combined with `Job/ExtendedRead` and
`Agent/ExtendedRead` to provide a fully read-only Jenkins experience,
where users can inspect configuration without being able to make changes.

For example, a user with `Overall/SystemRead` permission can view
global Jenkins settings such as installed plugins, tool configurations,
and system diagnostics, but cannot change or apply any system-level
configuration.

It is primarily intended to be used when the Jenkins configuration is managed externally, e.g. using the plugin:configuration-as-code/[Configuration as Code] plugin.
...
Read-only Jenkins is a configuration model where users can view job,
agent, and system configuration without being able to modify it.

This model is commonly used when Jenkins configuration is managed
externally, for example using Jenkins Configuration as Code (JCasC).

Read-only Jenkins is implemented by combining the following permissions:
* `Overall/SystemRead`
* `Job/ExtendedRead`
* `Agent/ExtendedRead`

This approach allows safer access for developers and operators to
inspect configuration, debug issues, and verify plugin settings
without requiring administrative privileges.

NOTE: Not all plugins fully support read-only mode yet.
Some plugin configuration pages may still appear editable even when
`Overall/SystemRead` and `ExtendedRead` permissions are granted.
